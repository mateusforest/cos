...
import {
  Search, Bell, Command, Menu, User, Settings, ExternalLink, LogOut,
} from "lucide-react"
...
export function PortalHeader({ placeholder = "Pergunte ao COS..." }: { placeholder?: string }) {
  ...
  const unreadCount = notifications.filter((n) => n.unread && !reads.includes(n.id)).length

  const submitSearch = (e: React.FormEvent) => {
    e.preventDefault()
    if (query.trim()) router.push(`/portal/conversas?q=${encodeURIComponent(query.trim())}`)
  }
      ...
      <div className="flex items-center gap-2 sm:gap-4 ml-4 sm:ml-6">
              ...
              <>
                ...
                >
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                    ...
                    <button
                      onClick={() => setReads(notifications.map((n) => n.id))}
                      className="text-xs text-muted-foreground hover:text-foreground transition-colors"
                    >
                      Marcar todas como lidas
                    </button>
                  </div>
                  <div className="max-h-80 overflow-y-auto px-4 py-10 text-center text-sm text-muted-foreground">
        ...
        <div className="relative">
          <button
            ...
            className="w-9 h-9 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white text-sm font-medium cursor-pointer"
              ...
              <>
                ...
                >
                  ...
                  <div className="border-t border-gray-100 mt-1 pt-1">
                    <Link href="/" onClick={() => setOpenMenu(null)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-colors">
